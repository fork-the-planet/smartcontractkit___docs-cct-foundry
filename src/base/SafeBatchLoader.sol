...
///         `SafeBatchEmitter.write`. This is what lets independently emitted per-operation batches be
///         COMPOSED: load each file, `CctActions.concat` them, and hand the merged `Call[]` to the
...
library SafeBatchLoader {
    ...
    function load(string memory path)
        internal
        view
    ...
    function loadAndValidate(string memory path, uint256 expectedChainId, address expectedSafe)
        internal
        view
        returns (CctActions.Call[] memory calls)
    {
        (uint256 chainId, address safe, CctActions.Call[] memory loaded) = load(path);
        require(
    ...
    function loadMany(string[] memory paths, uint256 expectedChainId, address expectedSafe)
        internal
        view
        returns (CctActions.Call[] memory merged)
    {
        ...
        for (uint256 i = 0; i < paths.length; i++) {
            CctActions.Call[] memory calls = loadAndValidate(paths[i], expectedChainId, expectedSafe);
            merged = i == 0 ? calls : CctActions.concat(merged, calls);
        }
    }
