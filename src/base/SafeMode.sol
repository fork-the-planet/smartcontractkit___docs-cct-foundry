...
library SafeMode {
    ...
    function run(CctActions.Call[] memory calls) internal returns (string memory path) {
        address safe = vm.envAddress("SAFE_ADDRESS");
        string memory batchName = vm.envOr("BATCH_NAME", string("cct-batch"));

        path = emitBatch(batchName, safe, calls);
    ...
    /// @return path the batch JSON file written under `batches/`.
    function emitBatch(string memory batchName, address safe, CctActions.Call[] memory calls)
        internal
        returns (string memory path)
    {
        ...
        SafeBatchEmitter.write(path, block.chainid, safe, batchName, "CCT action-layer batch (docs-cct-foundry)", calls);
    ...
    function encodeForSafe(CctActions.Call[] memory calls)
        internal
        pure
    ...
    ///         Service needed.
    function execDirect(ISafe safe, CctActions.Call[] memory calls) internal {
        (address to, uint256 value, bytes memory data, uint8 operation) = encodeForSafe(calls);
        uint256 nonce = safe.nonce();

        bytes32 localHash = SafeTxHash.compute(
            block.chainid,
            address(safe),
            SafeTxHash.SafeTx({
                to: to,
